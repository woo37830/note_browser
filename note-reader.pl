...
opendir(DIR, $NOTE_DIR) or die "Can open dir $NOTE_DIR";
@raw_files = grep(/Notes_.*.txt$/,readdir(DIR));
closedir(DIR);
my $filename = "tmp.txt";
open(TMP, '>', $filename) or die "Could not open file '$filename' $!";
$PREV_DATE = "";
$PREV_DATA = "";
@files = sort @raw_files;
foreach $file (@files) {
